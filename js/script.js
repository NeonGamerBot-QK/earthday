(() => {
  ...
  const days = [
    {
      name: "Mon",
      active: false,
      text: "The usage of how to use renewable and nonrenewable energy.<br />",
    ...
    {
      name: "Wens",
      text: "Today we’re reminding members of the KCD community to recycle! Recycling is necessary to save our planet. It conserves energy and natural resources and reduces air pollution, water pollution, and greenhouse gasses!",
      title: "Reminder to recycle",
    },
    {
      name: "Thurs",
      title: "Items to conserve energy",
      text: "These are energy conserving items that are easy to purchase and install in your homes! \nLED Lights \n Motion Sensor Lights \n Smart Thermostat",
    },
    {
      name: "Fri",
      title: "Actions to conserve energy",
      text: "Today we’re talking about actions that conserve energy!\nYou can carpool to save gas, turn off lights when you don’t need them",
    },
  ];
  const visuals = [
    {
      ...
      badges: [
        ..."Morning Announcements, Morning Meetings, Campaign Visual, Bearcat Weekly".split(
          ","
        ),
      ],
    },
    {
      ...
      items: [
        {
          text: "LED lights\n   https://www.energy.gov/energysaver/led-lighting#:~:text=LED%20is%20a%20highly%20energy,times%20longer%2C%20than%20incandescent%20lighting. ",
          badges: "Social Media, Campaign Visual, Announcements".split(","),
        },
        {
          text: "Motion Sensor Lights https://ledlightinginfo.com/motion-sensor-lights-save-electricity",
          badges: "Announcements".split(" "),
        },
      ],
    },
  ].map((e) => {
    if (e.text) {
      const matches = e.text.match(
        /([\w+]+\:\/\/)?([\w\d-]+\.)*[\w-]+[\.\:]\w+([\/\?\=\&\#.]?[\w-]+)*\/?/gm
      );
      if (matches) {
        matches.forEach((match) => {
          e.text = e.text.replace(
            match,
            `<a href="${match}" target="_blank" class="hover:text-base-900 text-base-700 underline">${match}</a>`
          );
        });
      }
      return e;
  ...
  const energyconserv1 = [
    ...
    [
      ...
      {
        name: "Drive less",
        data: `Driving less can save energy and fuel for your car.\n While saving energy your are also helping the environment! `,
        img: `https://www.shutterstock.com/image-photo/happy-funny-man-driving-flying-260nw-350972801.jpg`,
      },
    ],
    [
      {
  ...
  fetch("/requests")
    ...
    .then((r) => {
      setTimeout(() => {
        ...
        document.getElementById("s_requests").innerHTML +=
          r.school_people_requests;
      }, 500);
    });
  window.addEventListener("load", async () => {
    ...
    const loadingScreen = await fetch("/views/loading.html").then((e) =>
      e.text()
    );
    ...
    switch (window.location.hash) {
      ...
      case "#/":
        ...
        break;
      case "#/socialmedia":
        fetch("/views/campaign/socialmedia.html")
          .then((r) => r.text())
          .then((data) => {
            render(data, { posts });
          });
        break;
      case "#/plan":
        fetch("/views/campaign/plan.html")
          .then((r) => r.text())
          .then((data) => {
            render(data, {});
          });
      ...
      case "#/info":
      case "#/authors":
        fetch("/views/credits.html")
          .then((r) => r.text())
          .then(async (data) => {
            render(data, {
              info: await fetch("/requests").then((r) => r.json()),
            });
            ...
            coolDiv.className =
              "hero p-5 m-5 font-bold text-xl text-center bg-base-100 mt-20 hover:pointer";
            let keys = "";
      ...
      case "#/energyconversion":
        fetch("/views/energyconservtion.html")
          .then((r) => r.text())
          .then((data) => {
        ...
        break;
      case "#/bearcatweekly":
        fetch("/views/campaign/bearcatweekly.html")
          .then((r) => r.text())
          .then((data) => {
            render(data);
          });
        break;
      case "#/weeklyagenda":
        fetch("/views/campaign/dailyagenda.html")
          .then((r) => r.text())
          .then((data) => {
      ...
      case "#/mondaymeeting":
        fetch("/views/campaign/mondaymeeting.html")
          .then((r) => r.text())
          .then((data) => {
            // alert(location.search)
            // days[params.get("q") ].active = true;
    ...
    btn.addEventListener("click", () => {
      ...
      if (open) {
        ...
        document.getElementById("menu-dropdown").className += [
          "transform",
          "hidden",
        ].join(" ");
      }
    ...
    const elements = Array.from(
      document.querySelectorAll("button.search-results")
    );
